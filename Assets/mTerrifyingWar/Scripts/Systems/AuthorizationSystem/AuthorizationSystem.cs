...
public class AuthorizationSystem : MonoBehaviour
{
    ...
    [SerializeField, BoxGroup("Registration")] private TMP_InputField _emailAutInputField;
    ...
    {
        if (IsInputEmpty(_loginRegInputField, _emailAutInputField, _passwordOneRegInputField, _passwordTwoRegInputField))
        { 
        ...
        if (_passwordOneRegInputField.text.Length < 8)
        {
            _notificationSystem.AddMessage("Пароль должен содержать минимум 8 символов", Color.red);
            return;
        }
